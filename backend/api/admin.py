...
async def _ingest_pdf(textbook_id: str, file_bytes: bytes, board: str, class_name: str, subject: str):
    ...
    try:
        ...
        for i in range(0, len(chunk_docs), batch_size):
            await db.chunks.insert_many(chunk_docs[i:i+batch_size])

        # Update textbook status
        await db.textbooks.update_one(
            {"_id": ObjectId(textbook_id)},
            {"$set": {"status": "ready", "chunk_count": len(chunk_docs)}}
        )
        print(f"[Ingest] Textbook {textbook_id}: {len(chunk_docs)} chunks ingested")
